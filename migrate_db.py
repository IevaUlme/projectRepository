...
logger.info('TicTacToe games DB migration service')
...
def mysql_create_migrations_table():
    ...
    try:
        ...
        result = cursor.execute(
            "CREATE TABLE `migrations` ( `id` INT NOT NULL AUTO_INCREMENT, `name` VARCHAR(255), `exec_ts` INT(10), `exec_dt` varchar(20), PRIMARY KEY (`id`))")
        connection.commit()
    except Error as e:
        logger.error(
            "CREATE TABLE `migrations` ( `id` INT NOT NULL AUTO_INCREMENT, `name` VARCHAR(255), `exec_ts` INT(10), `exec_dt` varchar(20), PRIMARY KEY (`id`))")
...
def mysql_migration_value_insert(name, exec_ts, exec_dt):
    ...
    try:
        cursor = connection.cursor()
        result = cursor.execute(
            "INSERT INTO `migrations` (`name`, `exec_ts`, `exec_dt`) VALUES ('" + str(name) + "', '" + str(
                exec_ts) + "', '" + str(exec_dt) + "')")
        connection.commit()
    except Error as e:
        logger.error("INSERT INTO `migrations` (`name`, `exec_ts`, `exec_dt`) VALUES ('" + str(name) + "', '" + str(
            exec_ts) + "', '" + str(exec_dt) + "')")
